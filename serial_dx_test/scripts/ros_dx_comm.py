...
def motor1cb(message):
  cp = motor.getPosition(1)
  received_current = motor.setTorque(int(message.data), 1)
  print "motor1 current : {} [A]".format(received_current)
  print "motor1 position : {}".format(cp)

def motor2cb(message):
  cp = motor.getPosition(2)
  received_current = motor.setTorque(int(message.data), 2)
  print "motor2 current : {} [A]".format(received_current)
  print "motor2 position : {}".format(cp)

rospy.init_node("dx_comm")
motor = MX64Control(b'/dev/ttyS0', 57600, [1, 2])

sub1 = rospy.Subscriber("current_controller1", Float32, motor1cb)
sub2 = rospy.Subscriber("current_controller2", Float32, motor2cb)
rospy.spin()
