...
with open("/home/ubuntu/workspace/src/fluid_network_synergyhand/bellows_sensor/scripts/weights0117_py2", "rb") as f:
  w = pickle.load(f)
...
def loop(pot_min, filepath, mmlist=[0, 10], max_pressure_list=[10, 20, 30]):
  global potangle, potangle2, stretch, mz, pressure, servo_s
  ...
  for mm in mmlist:
    ...
    for n, max_pressure in enumerate(max_pressure_list):
      ...
      print("pressure -> 20")
      servo_pos_control(goal=15, diff_type="pressure")
      time.sleep(1)
      print("pressure -> -18")
      servo_pos_control(goal=-15, diff_type="pressure")
      ...
      with open(os.path.join(filepath, str(mm), str(n)+".csv"), "w") as f: 
        writer = csv.writer(f)
        writer.writerow(["Time", "Syringe Servo Pulses", "Potentiometer", "Potentiometer2","Stretch", "Pressure", "Moment", "Prediction", "Extraction", "Injection", "Stop"])
        ...
        for goal, diff_type in [(init_srv-maxpulse, "servo"), (-15, "pressure")]:
          for i in range(300):
            ...
            writer.writerow([time.time()-init_time, t_srv, potangle, potangle2, stretch, pressure, mz, pred] + servo_s)
...
if __name__ == "__main__":
  ...
  # mmlist = range(75, -5, -5)
  print(mmlist)
  max_pressure_list = [25, 25, 25]


  try:
    loop(pot_min=280, filepath=filepath, mmlist=mmlist, max_pressure_list=max_pressure_list)
